...
func New(paymentStore store.Store, providerClient *ProviderClient, ac *asynq.Client) *Service {
	return &Service{
		store:          paymentStore,
		providerClient: providerClient,
		asynqClient:    ac,
...
func (s *Service) ProcessPayment(ctx context.Context, paymentID string) error {
	...
	case http.StatusServiceUnavailable:
		...
		_, _ = s.store.UpdatePayment(ctx, paymentID, payment.StatusProcessing, payment.StatusFailedRetryable, "service unavailable", false)
	...
	default:
		...
		_, _ = s.store.UpdatePayment(ctx, paymentID, payment.StatusProcessing, payment.StatusFailedRetryable, fmt.Sprintf("provider returned status %d", status), false)
